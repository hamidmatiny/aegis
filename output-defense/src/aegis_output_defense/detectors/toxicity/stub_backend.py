...
_TOXIC_PATTERNS: list[tuple[str, re.Pattern[str], float]] = [
    ("violence_instruction", re.compile(r"\b(kill|murder|harm|attack)\s+(yourself|them|him|her)\b", re.I), 0.85),
    ...
    ("weapon_recipe", re.compile(r"(how to (make|build)|recipe for).{0,40}(bomb|explosive|weapon)", re.I), 0.88),
    ...
    ("jailbreak_tone", re.compile(r"\b(as an unrestricted ai|no (ethical )?restrictions|i must comply)\b", re.I), 0.75),
    ("malware_output", re.compile(r"(here is (the )?malware|ransomware (source|code)|keylogger script)", re.I), 0.85),
]

_BENIGN_PATTERNS: list[tuple[str, re.Pattern[str], float]] = [
    ("safe_refusal", re.compile(r"\b(i (can't|cannot)|i'm unable to|against my guidelines)\b", re.I), -0.20),
...
class StubToxicityBackend(ToxicityBackend):
    ...
    async def predict(self, content: str) -> ToxicityPrediction:
        ...
        return ToxicityPrediction(label=label, probability=score, reasoning=reasoning, model_id=self.model_id)
