...
`CORP_READONLY_TOKEN` is generated by `./scripts/generate-credentials.sh` (never overwrite if already set). Wire it into compose via `docker-compose.yml` (`CORP_READONLY_TOKEN`). Use it only for external read-only briefing agents — it cannot call POST run/decide.
