...
| **Dashboard auth** | HTTP basic auth via `AEGIS_DASHBOARD_USER` / `AEGIS_DASHBOARD_PASSWORD` (default username `admin`; password is generated by `./scripts/generate-credentials.sh` or emitted once on container startup — no static `changeme` default) |
