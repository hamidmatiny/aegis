...
Single implementation shared by smb-copilot and corp-orchestrator so the
admin/customer cookie check cannot drift between services.
"""
...
def _session_secret() -> bytes:
    secret = os.environ.get("SMB_SESSION_SECRET") or ""
    if not secret:
        raise RuntimeError("SMB_SESSION_SECRET must be set")
    return secret.encode("utf-8")


def _redis_url() -> str:
    return os.environ.get("REDIS_URL") or "redis://:aegis_redis_dev@127.0.0.1:6379/0"
...
def _sign_token(token: str) -> str:
    digest = hmac.new(_session_secret(), token.encode("utf-8"), hashlib.sha256).hexdigest()
...
def _verify_signed(value: str) -> str | None:
    ...
    expected = hmac.new(_session_secret(), token.encode("utf-8"), hashlib.sha256).hexdigest()
...
def clear_session_cookie(response: Response, *, path: str = "/") -> None:
    response.delete_cookie(key=SESSION_COOKIE, path=path)
